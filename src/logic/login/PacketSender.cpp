...
void PacketSender::Send(const s32 gate, const s32 actorId, const s32 msgId, const OBuffer& buf) {
	s32 header[2];
	header[0] = msgId;
	header[1] = buf.GetSize() + sizeof(s32) * 2;

	_harbor->PrepareSend(user_node_type::GATE, gate, framework_proto::TRANSMIT_TO_ACTOR, sizeof(s32) * 2 + buf.GetSize() + sizeof(s64));
...
void PacketSender::Brocast(const std::unordered_map<s32, std::vector<s64>>& actors, const s32 msgId, const OBuffer& buf) {
	s32 header[2];
	header[0] = msgId;
	header[1] = buf.GetSize() + sizeof(s32) * 2;
	...
	for (auto itr = actors.begin(); itr != actors.end(); ++itr) {
		if (!itr->second.empty()) {
			_harbor->PrepareSend(user_node_type::GATE, itr->first, framework_proto::TRANSMIT_TO_ACTOR, sizeof(s32) * 2 + buf.GetSize() + sizeof(s64) * (s32)itr->second.size());
