...
void MD5::update(const std::string& str) {
	update((const u8*)str.c_str(), str.length());
}
...
/* Convert u8 array to hex std::string. */
std::string MD5::bytesToHexString(const u8* input, size_t length) {
...
std::string MD5::toString() {
	return std::move(bytesToHexString(digest(), 16));
}
