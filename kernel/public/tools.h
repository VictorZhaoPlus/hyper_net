...
namespace tools {
	...
	inline s32 CalcStringUniqueId(const char * str) {
		s32 seed = 131;
		s32 hash = 0;
		while (*str) {
			hash = hash * seed + (*str++);
	...
	KERNEL_API void ListFileInDirection(const char * path, const char * extension, const std::function<void (const char *, const char *)>& f);
}
#endif // __TOOLS_H__
